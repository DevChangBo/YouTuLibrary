...
import com.example.youtulibrary.youtu.Base64Util;
import com.example.youtulibrary.youtu.YoutuSign;
...
public class YTServerAPI {
    ...
    public final static String API_URL = "https://api.youtu.qq.com/youtu/";
...
//		data.put("url", "http://182.150.21.207:8089/expressway/a/userfiles/1/aaa.jpg");
    ...
    /**
     * 上传访问驾驶证/行驶证（0表示行驶证识别，1表示驾驶证识别）
     */
    public void driverlicenseocr(Bitmap bitmap, int cardType)throws IOException, JSONException, KeyManagementException, NoSuchAlgorithmException{
        JSONObject data = new JSONObject();
        String imageData = bitmapToBase64(bitmap);
        data.put("image", imageData);
        data.put("app_id", m_appid);
        ...
        if (m_use_youtu) {
            sendHttpRequest(data, "ocrapi/driverlicenseocr");
        }else{
            sendHttpsRequest(data, "ocrapi/driverlicenseocr");
        }
    }
     ...
     * @throws NoSuchAlgorithmException
     */
    public void plateocrCar(Bitmap bitmap)throws IOException, JSONException, KeyManagementException, NoSuchAlgorithmException{
        JSONObject data = new JSONObject();
        String imageData = bitmapToBase64(bitmap);
        data.put("image", imageData);
        data.put("app_id", m_appid);
        ...
        if (m_use_youtu) {
            sendHttpRequest(data, "ocrapi/plateocr");
        }else{
            sendHttpsRequest(data, "ocrapi/plateocr");
        }
    }
